...
public class ACTrie {
    ...
    private void initFailover() {
        ...
        Map<Character, ACTrieNode> children = root.children();
        ...
        while (!queue.isEmpty()) {
            ...
            for (Map.Entry<Character, ACTrieNode> entry : parentNode.children().entrySet()) {
                ACTrieNode childNode = entry.getValue();
                ACTrieNode failover = parentNode.failover();
    ...
    public List<MatchResult> matches(String text) {
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            while (!walkNode.hasChild(c) && walkNode.failover() != null) {
            ...
            if (walkNode.hasChild(c)) {
                ...
                // 检索到了敏感词
                if (walkNode.isLeaf()) {
                    result.add(new MatchResult(i - walkNode.depth() + 1, i + 1));
